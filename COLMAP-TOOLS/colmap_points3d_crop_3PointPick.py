...
class ColmapCropGUI(tk.Tk):
    ...
    def _on_mouse_release(self, event):
        if self._pick_mode and event.button == 1 and event.inaxes == self._ax:
    ...
    def _show_transform_window(self, preset_rx=0.0, preset_ry=0.0, preset_rz=0.0):
        ...
        pad = tk.Frame(win, bg="#16213e", padx=16, pady=10)
        ...
        tk.Label(pad,
                 ...
                 font=("Courier New", 8), fg=status_color, bg="#16213e"
                 ).pack(anchor="w", pady=(0, 4))
        if n_img == 0:
            tk.Label(pad,
                     text="Tip: use  📁 Open Scene  to also load cameras + images",
                     font=("Courier New", 7), fg="#556677", bg="#16213e"
        ...
        for axis in ("TX", "TY", "TZ"):
            ...
            tk.Label(row, text=f"{axis}:", width=5,
                     font=("Courier New", 9), fg="#82b6d9", bg="#16213e"
                     ).pack(side=tk.LEFT)
        ...
        for axis, preset in [("RX", preset_rx), ("RY", preset_ry), ("RZ", preset_rz)]:
            ...
            row.pack(fill=tk.X, pady=2)
            tk.Label(row, text=f"{axis}:", width=5,
                     font=("Courier New", 9), fg="#a0e77d", bg="#16213e"
                     ).pack(side=tk.LEFT)
        ...
        tk.Label(scale_row, text="Scale:", width=6,
                 font=("Courier New", 9), fg="#ef8677", bg="#16213e"
                 ).pack(side=tk.LEFT)
    ...
    def _redraw_keep_picks(self):
        ...
        for i, pt in enumerate(self._picked_pts):
            ...
            ax.text(px, py, pz, f"  {self._PICK_LABELS[i]}",
                    ...
                    fontfamily="Courier New", zorder=11)
    ...
    def _show_plane_result_panel(self, normal, R):
        ...
        def _hdr(text):
            tk.Label(pad, text=text, font=("Courier New", 9, "bold"),
        ...
        for i, (pt, lbl, col) in enumerate(zip(
            ...
            tk.Label(pad, text=f"{lbl}:  {pt[0]:+.4f}  {pt[1]:+.4f}  {pt[2]:+.4f}",
                     font=("Courier New", 8), fg=col, bg="#16213e", anchor="w"
                     ).pack(fill=tk.X)

        _hdr("PLANE NORMAL")
        tk.Label(pad,
                 text=f"N:  {normal[0]:+.4f}  {normal[1]:+.4f}  {normal[2]:+.4f}",
                 font=("Courier New", 8), fg="#82b6d9", bg="#16213e", anchor="w"
                 ).pack(fill=tk.X)
        ...
        rot_lbl = tk.Label(pad,
            text=f"RX {rx:+.4f}°   RY {ry:+.4f}°   RZ {rz:+.4f}°\n"
                 "(aligns plane → +Y)",
            font=("Courier New", 8), fg="#a0e77d", bg="#16213e", justify="left")
